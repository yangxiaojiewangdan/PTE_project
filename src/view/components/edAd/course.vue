	...
	<div>
		...
		<Modal v-model="AddDepartment" width="800" title="添加课程信息" :mask-closable="false" :styles="{top: '20px'}">
			<p slot="header" style="text-align: left; line-height: 1;">
				<span v-if="add">添加课程</span>
				<span v-if="see">修改课程</span>
			</p>
			<Form ref="CourseForm" :model="CourseForm" :rules="ruleValidate" :label-width="95" inline>
				<Row>
					<Col span="10">
		...
		<Modal v-model="AddRoyalty" width="700" title="添加课程阶段信息" :mask-closable="false">
			<Form ref="stageForm" :model="stageForm" :rules="ruleValidate" :label-width="95" inline>
				<Row>
	...
	export default {
		...
		methods: {
			...
			handleSubmit(name) {
				this.$refs[name].validate(valid => {
					if(
						(valid && this.CourseForm.Id == undefined) ||
						this.CourseForm.Id == ""
					) {
						//保存阶段信息
						...
						Create(this.Interface, this.CourseForm)
							.then(res => {
						...
						Update(this.Interface, this.CourseForm)
							.then(res => {
								console.log(res.data);
			...
			stagehandleSubmit(name) {
				this.$refs[name].validate(valid => {
					if(
						(valid && this.stageForm.Id == undefined) ||
						this.stageForm.Id == ""
					) {
			...
			//确定删除
			ok() {
				BatchDelete(this.Interface, this.BatchDeleteList)
					.then(res => {
						console.log(res);
						this.reload();
					})
		...
		mounted() {
			...
			GetEntities(this.Interface, this.CourseData1)
				...
				//业务类型
				GetEntities("BusinessType", this.CourseData1)
				.then(res => {
